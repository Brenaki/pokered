# Data Wild 2

> 17 nodes · cohesion 0.12

## Key Concepts

- **WildDataPointers** (62 connections) — `data/wild/grass_water.asm`
- **data/wild/maps/MtMoon1F.asm** (3 connections) — `data/wild/maps/MtMoon1F.asm`
- **data/wild/maps/MtMoonB1F.asm** (3 connections) — `data/wild/maps/MtMoonB1F.asm`
- **data/wild/maps/PokemonTower7F.asm** (3 connections) — `data/wild/maps/PokemonTower7F.asm`
- **data/wild/maps/Route11.asm** (3 connections) — `data/wild/maps/Route11.asm`
- **data/wild/maps/Route1.asm** (3 connections) — `data/wild/maps/Route1.asm`
- **data/wild/maps/SafariZoneEast.asm** (3 connections) — `data/wild/maps/SafariZoneEast.asm`
- **data/wild/maps/SeafoamIslandsB3F.asm** (3 connections) — `data/wild/maps/SeafoamIslandsB3F.asm`
- **CeruleanCave1FWildMons** (2 connections) — `data/wild/maps/CeruleanCave1F.asm`
- **MtMoon1FWildMons** (2 connections) — `data/wild/maps/MtMoon1F.asm`
- **MtMoonB1FWildMons** (2 connections) — `data/wild/maps/MtMoonB1F.asm`
- **PokemonMansion3FWildMons** (2 connections) — `data/wild/maps/PokemonMansion3F.asm`
- **PokemonTower7FWildMons** (2 connections) — `data/wild/maps/PokemonTower7F.asm`
- **Route11WildMons** (2 connections) — `data/wild/maps/Route11.asm`
- **Route1WildMons** (2 connections) — `data/wild/maps/Route1.asm`
- **SafariZoneEastWildMons** (2 connections) — `data/wild/maps/SafariZoneEast.asm`
- **SeafoamIslandsB3FWildMons** (2 connections) — `data/wild/maps/SeafoamIslandsB3F.asm`

## Relationships

- [Data Wild](Data_Wild.md) (8 shared connections)
- [Data Wild 3](Data_Wild_3.md) (7 shared connections)
- [Data Wild 5](Data_Wild_5.md) (1 shared connections)
- [Data Wild 6](Data_Wild_6.md) (1 shared connections)
- [Data Wild 7](Data_Wild_7.md) (1 shared connections)
- [Data Wild 8](Data_Wild_8.md) (1 shared connections)
- [Data Wild 9](Data_Wild_9.md) (1 shared connections)
- [Data Wild 10](Data_Wild_10.md) (1 shared connections)
- [Data Wild 11](Data_Wild_11.md) (1 shared connections)
- [Data Wild 12](Data_Wild_12.md) (1 shared connections)
- [Data Wild 13](Data_Wild_13.md) (1 shared connections)
- [Data Wild 14](Data_Wild_14.md) (1 shared connections)

## Source Files

- `data/wild/grass_water.asm`
- `data/wild/maps/CeruleanCave1F.asm`
- `data/wild/maps/MtMoon1F.asm`
- `data/wild/maps/MtMoonB1F.asm`
- `data/wild/maps/PokemonMansion3F.asm`
- `data/wild/maps/PokemonTower7F.asm`
- `data/wild/maps/Route1.asm`
- `data/wild/maps/Route11.asm`
- `data/wild/maps/SafariZoneEast.asm`
- `data/wild/maps/SeafoamIslandsB3F.asm`

## Audit Trail

- EXTRACTED: 66 (100%)
- INFERRED: 0 (0%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*