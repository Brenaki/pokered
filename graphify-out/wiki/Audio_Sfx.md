# Audio Sfx

> 204 nodes · cohesion 0.01

## Key Concepts

- **audio/sfx** (324 connections)
- **audio/sfx/cry01_1.asm** (5 connections) — `audio/sfx/cry01_1.asm`
- **audio/sfx/cry10_1.asm** (5 connections) — `audio/sfx/cry10_1.asm`
- **audio/sfx/cry14_1.asm** (5 connections) — `audio/sfx/cry14_1.asm`
- **audio/sfx/cry1c_1.asm** (5 connections) — `audio/sfx/cry1c_1.asm`
- **audio/sfx/cry1e_1.asm** (5 connections) — `audio/sfx/cry1e_1.asm`
- **audio/sfx/psychic_m.asm** (5 connections) — `audio/sfx/psychic_m.asm`
- **audio/sfx/battle_24.asm** (4 connections) — `audio/sfx/battle_24.asm`
- **audio/sfx/purchase_3.asm** (4 connections) — `audio/sfx/purchase_3.asm`
- **audio/sfx/save_3.asm** (4 connections) — `audio/sfx/save_3.asm`
- **audio/sfx/arrow_tiles_3.asm** (3 connections) — `audio/sfx/arrow_tiles_3.asm`
- **audio/sfx/battle_09.asm** (3 connections) — `audio/sfx/battle_09.asm`
- **audio/sfx/battle_0c.asm** (3 connections) — `audio/sfx/battle_0c.asm`
- **audio/sfx/battle_0d.asm** (3 connections) — `audio/sfx/battle_0d.asm`
- **audio/sfx/battle_0e.asm** (3 connections) — `audio/sfx/battle_0e.asm`
- **audio/sfx/battle_0f.asm** (3 connections) — `audio/sfx/battle_0f.asm`
- **audio/sfx/battle_13.asm** (3 connections) — `audio/sfx/battle_13.asm`
- **audio/sfx/battle_16.asm** (3 connections) — `audio/sfx/battle_16.asm`
- **audio/sfx/battle_18.asm** (3 connections) — `audio/sfx/battle_18.asm`
- **audio/sfx/battle_1b.asm** (3 connections) — `audio/sfx/battle_1b.asm`
- **audio/sfx/battle_1c.asm** (3 connections) — `audio/sfx/battle_1c.asm`
- **audio/sfx/battle_21.asm** (3 connections) — `audio/sfx/battle_21.asm`
- **audio/sfx/battle_22.asm** (3 connections) — `audio/sfx/battle_22.asm`
- **audio/sfx/battle_25.asm** (3 connections) — `audio/sfx/battle_25.asm`
- **audio/sfx/battle_26.asm** (3 connections) — `audio/sfx/battle_26.asm`
- *... and 179 more nodes in this community*

## Relationships

- [Audio Sfx 2](Audio_Sfx_2.md) (76 shared connections)
- [Audio Sfx 3](Audio_Sfx_3.md) (2 shared connections)
- [Audio Sfx 137](Audio_Sfx_137.md) (1 shared connections)
- [Audio Sfx 138](Audio_Sfx_138.md) (1 shared connections)
- [Audio Sfx 139](Audio_Sfx_139.md) (1 shared connections)
- [Audio Sfx 140](Audio_Sfx_140.md) (1 shared connections)
- [Audio Sfx 141](Audio_Sfx_141.md) (1 shared connections)
- [Audio Sfx 4](Audio_Sfx_4.md) (1 shared connections)
- [Audio Sfx 5](Audio_Sfx_5.md) (1 shared connections)
- [Audio Sfx 142](Audio_Sfx_142.md) (1 shared connections)
- [Audio Sfx 6](Audio_Sfx_6.md) (1 shared connections)
- [Audio Sfx 7](Audio_Sfx_7.md) (1 shared connections)

## Source Files

- `audio/sfx/arrow_tiles_3.asm`
- `audio/sfx/battle_09.asm`
- `audio/sfx/battle_0c.asm`
- `audio/sfx/battle_0d.asm`
- `audio/sfx/battle_0e.asm`
- `audio/sfx/battle_0f.asm`
- `audio/sfx/battle_13.asm`
- `audio/sfx/battle_16.asm`
- `audio/sfx/battle_18.asm`
- `audio/sfx/battle_1b.asm`
- `audio/sfx/battle_1c.asm`
- `audio/sfx/battle_21.asm`
- `audio/sfx/battle_22.asm`
- `audio/sfx/battle_24.asm`
- `audio/sfx/battle_25.asm`
- `audio/sfx/battle_26.asm`
- `audio/sfx/collision_1.asm`
- `audio/sfx/collision_3.asm`
- `audio/sfx/cry01_1.asm`
- `audio/sfx/cry10_1.asm`

## Audit Trail

- EXTRACTED: 432 (100%)
- INFERRED: 0 (0%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*