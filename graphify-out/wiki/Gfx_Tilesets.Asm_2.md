# Gfx Tilesets.Asm 2

> 30 nodes · cohesion 0.13

## Key Concepts

- **gfx/tilesets.asm** (52 connections) — `gfx/tilesets.asm`
- **Tilesets 2 [ROMX]** (30 connections) — `gfx/tilesets.asm`
- **Facility_Block** (3 connections) — `gfx/tilesets.asm`
- **Facility_GFX** (3 connections) — `gfx/tilesets.asm`
- **Forest_Block** (3 connections) — `gfx/tilesets.asm`
- **Forest_GFX** (3 connections) — `gfx/tilesets.asm`
- **Gate_Block** (3 connections) — `gfx/tilesets.asm`
- **Gate_GFX** (3 connections) — `gfx/tilesets.asm`
- **Gym_Block** (3 connections) — `gfx/tilesets.asm`
- **Gym_GFX** (3 connections) — `gfx/tilesets.asm`
- **Pokecenter_Block** (3 connections) — `gfx/tilesets.asm`
- **Pokecenter_GFX** (3 connections) — `gfx/tilesets.asm`
- **gfx/blocksets/facility.bst** (2 connections) — `gfx/blocksets/facility.bst`
- **gfx/blocksets/forest.bst** (2 connections) — `gfx/blocksets/forest.bst`
- **gfx/blocksets/gate.bst** (2 connections) — `gfx/blocksets/gate.bst`
- **gfx/blocksets/gym.bst** (2 connections) — `gfx/blocksets/gym.bst`
- **gfx/blocksets/pokecenter.bst** (2 connections) — `gfx/blocksets/pokecenter.bst`
- **gfx/tilesets/facility.2bpp** (2 connections) — `gfx/tilesets/facility.2bpp`
- **gfx/tilesets/forest.2bpp** (2 connections) — `gfx/tilesets/forest.2bpp`
- **gfx/tilesets/gate.2bpp** (2 connections) — `gfx/tilesets/gate.2bpp`
- **gfx/tilesets/gym.2bpp** (2 connections) — `gfx/tilesets/gym.2bpp`
- **gfx/tilesets/pokecenter.2bpp** (2 connections) — `gfx/tilesets/pokecenter.2bpp`
- **Dojo_Block** (2 connections) — `gfx/tilesets.asm`
- **Dojo_GFX** (2 connections) — `gfx/tilesets.asm`
- **ForestGate_Block** (2 connections) — `gfx/tilesets.asm`
- *... and 5 more nodes in this community*

## Relationships

- [Gfx Tilesets.Asm](Gfx_Tilesets.Asm.md) (17 shared connections)
- [Gfx Tilesets.Asm 3](Gfx_Tilesets.Asm_3.md) (15 shared connections)
- [Bank ROMX](Bank_ROMX.md) (1 shared connections)

## Source Files

- `gfx/blocksets/facility.bst`
- `gfx/blocksets/forest.bst`
- `gfx/blocksets/gate.bst`
- `gfx/blocksets/gym.bst`
- `gfx/blocksets/pokecenter.bst`
- `gfx/tilesets.asm`
- `gfx/tilesets/facility.2bpp`
- `gfx/tilesets/forest.2bpp`
- `gfx/tilesets/gate.2bpp`
- `gfx/tilesets/gym.2bpp`
- `gfx/tilesets/pokecenter.2bpp`

## Audit Trail

- EXTRACTED: 90 (100%)
- INFERRED: 0 (0%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*